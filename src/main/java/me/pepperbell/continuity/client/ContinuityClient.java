...
public class ContinuityClient {
  ...
  private void onAddPackFinders(AddPackFindersEvent event) {
    if (event.getPackType() == PackType.CLIENT_RESOURCES) {
      registerBuiltinResourcePack(
          event, "default", Component.translatable("resourcePack.continuity.default.name"));
      registerBuiltinResourcePack(
          event,
  ...
  private static void registerBuiltinResourcePack(
      AddPackFindersEvent event, String packName, Component title) {
    try {
      Path path =
          ModList.get().getModFileById(ID).getFile().findResource("resourcepacks", packName);
      Pack pack =
          Pack.readMetaAndCreate(
              "builtin/continuity/" + packName,
              title,
              false,
  ...
  private void registerLoaders() {
    ...
    loader =
        createCustomLoader(
            OrientedConnectingOverlayCtmProperties::new,
            new TileAmountValidator.Exactly<>(7),
            new SimpleOverlayQuadProcessor.Factory<>(
  ...
  private static <T extends CtmProperties> CtmLoader<T> createLoader(
      CtmProperties.Factory<T> propertiesFactory,
  ...
  private static <T extends BaseCtmProperties> CtmLoader<T> createLoader(
      CtmProperties.Factory<T> propertiesFactory,
      TileAmountValidator<T> validator,
      QuadProcessor.Factory<T> processorFactory,
      boolean isValidForMultipass) {
    return createLoader(
        wrapWithOptifineOnlyCheck(
            TileAmountValidator.wrapFactory(
                BaseCtmProperties.wrapFactory(propertiesFactory), validator)),
        processorFactory,
        new BaseCachingPredicates.Factory<>(isValidForMultipass));
  }
