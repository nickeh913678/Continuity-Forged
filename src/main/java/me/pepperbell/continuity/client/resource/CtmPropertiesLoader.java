...
public class CtmPropertiesLoader {
    ...
    try {
      for (String namespace : pack.getNamespaces(PackType.CLIENT_RESOURCES)) {
        try {
                ...
                if (resourceId.getPath().endsWith(".properties")) {
                  try (InputStream stream = inputSupplier.get()) {
                    ...
                    ContinuityClient.LOGGER.error(
                        "Failed to load CTM properties from file '"
                            + resourceId
                            + "' in pack '"
                            + pack.packId()
                            + "'",
          ...
          ContinuityClient.LOGGER.error(
              "Failed to list CTM resources in namespace '"
                  + namespace
                  + "' from pack '"
                  + pack.packId()
                  + "'",
      ...
      ContinuityClient.LOGGER.error(
          "Failed to enumerate namespaces from pack '" + pack.packId() + "'", e);
    }
  }

  private void load(
      Properties properties, ResourceLocation resourceId, PackResources pack, int packPriority) {
  ...
  private <T extends CtmProperties> void load(
      CtmLoader<T> loader,
    ...
    T ctmProperties =
        loader
            ...
            .createProperties(properties, resourceId, pack, packPriority, resourceManager, method);
    if (ctmProperties != null) {
      ...
      for (Material spriteId : ctmProperties.getTextureDependencies()) {
        Set<ResourceLocation> atlasTextureDependencies =
            textureDependencies.computeIfAbsent(
                spriteId.atlasLocation(), id -> new ObjectOpenHashSet<>());
        atlasTextureDependencies.add(spriteId.texture());
      }
    }
  }

  private record LoadingContainer<T extends CtmProperties>(CtmLoader<T> loader, T properties)
      implements Comparable<LoadingContainer<?>> {
    public QuadProcessors.ProcessorHolder toProcessorHolder(
        Function<Material, TextureAtlasSprite> textureGetter) {
      QuadProcessor processor =
          loader.getProcessorFactory().createProcessor(properties, textureGetter);
      CachingPredicates predicates =
          loader.getPredicatesFactory().createPredicates(properties, textureGetter);
  ...
  public static class LoadingResult {
    ...
    private LoadingResult(
        List<LoadingContainer<?>> containers,
        Map<ResourceLocation, Set<ResourceLocation>> textureDependencies) {
      this.containers = containers;
      this.textureDependencies = textureDependencies;
    }

    public List<QuadProcessors.ProcessorHolder> createProcessorHolders(
        Function<Material, TextureAtlasSprite> textureGetter) {
