...
public final class CustomBlockLayers {
  ...
  private static final Predicate<BlockState>[] EMPTY_LAYER_PREDICATES =
      new Predicate[BlockLayer.VALUES.length];

  @SuppressWarnings("unchecked")
  private static final Predicate<BlockState>[] LAYER_PREDICATES =
      new Predicate[BlockLayer.VALUES.length];
    ...
    if (optionalResource.isPresent()) {
      ...
      try (InputStream inputStream = resource.open()) {
        ...
        ContinuityClient.LOGGER.error(
            "Failed to load custom block layers from file '"
                + LOCATION
                + "' from pack '"
  ...
  private static void reload(Properties properties, ResourceLocation fileLocation, String packId) {
    for (BlockLayer blockLayer : BlockLayer.VALUES) {
      ...
      Predicate<BlockState> predicate =
          PropertiesParsingHelper.parseBlockStates(properties, propertyKey, fileLocation, packId);
  ...
  public static class ReloadListener {
    public static void init() {
      // Registration happens via BakedModelManagerReloadExtension
    }
