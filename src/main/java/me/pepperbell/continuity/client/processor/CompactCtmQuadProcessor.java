...
public class CompactCtmQuadProcessor extends AbstractQuadProcessor {
  ...
  @Nullable protected TextureAtlasSprite[] replacementSprites;

  public CompactCtmQuadProcessor(
      TextureAtlasSprite[] sprites,
      ProcessingPredicate processingPredicate,
      ConnectionPredicate connectionPredicate,
      boolean innerSeams,
      OrientationMode orientationMode,
      @Nullable TextureAtlasSprite[] replacementSprites) {
  ...
  public ProcessingResult processQuadInner(
    ...
    int connections =
        CtmSpriteProvider.getConnections(
            directions,
            connectionPredicate,
            innerSeams,
            mutablePos,
    ...
    // For non-unit-square quads (e.g., partial block faces from LittleTiles slopes),
    // skip splitting to avoid visible seam artifacts on non-planar geometry.
    // The no-split path uses majority vote across all orientation-adjusted quadrants.
    boolean nonUnitSquareSkipped = false;
    if ((uSplit | vSplit) && !QuadUtil.isQuadUnitSquare(quad)) {
      uSplit = false;
      vSplit = false;
      nonUnitSquareSkipped = true;
    }
    ...
    if (uSplit & vSplit) {
      ...
      if (split01 & split12 & split23 & split30) {
        ...
        vertexContainer.vertex01.setLerped(
            delta01, vertexContainer.vertex0, vertexContainer.vertex1);
        vertexContainer.vertex12.setLerped(
            delta12, vertexContainer.vertex1, vertexContainer.vertex2);
        vertexContainer.vertex23.setLerped(
            delta23, vertexContainer.vertex2, vertexContainer.vertex3);
        vertexContainer.vertex30.setLerped(
            delta30, vertexContainer.vertex3, vertexContainer.vertex0);
        vertexContainer.vertex4.setLerped(
            delta4, vertexContainer.vertex01, vertexContainer.vertex23);
        ...
        if (splits == 2) {
          ...
          if (!split01) {
            ...
            vertexContainer.vertex30.setLerped(
                delta30, vertexContainer.vertex3, vertexContainer.vertex0);
            vertexContainer.vertex4.setLerped(
                delta4, vertexContainer.vertex12, vertexContainer.vertex30);
      ...
      int[] quadrantIndexMap = QUADRANT_INDEX_MAPS[orientation];
      if (nonUnitSquareSkipped) {
        // Non-unit-square face (e.g. LittleTiles slope): pick best single sprite via
        // majority vote across all quadrants, matching the full CTM approach.
        int s0 = getSpriteIndex(quadrantIndexMap[0], connections);
        int s1 = getSpriteIndex(quadrantIndexMap[1], connections);
        int s2 = getSpriteIndex(quadrantIndexMap[2], connections);
        int s3 = getSpriteIndex(quadrantIndexMap[3], connections);
        ...
        tryInterpolate(quad, sprite, best);
      } else {
        // Normal no-split: UV stays in one quadrant. Map UV position to the vertex
        // index (0-3) so it aligns with the split path's quadrantIndexMap convention.
        // In sprite UV space: vn<0 = top of sprite = top of face (vertex1/vertex2 side),
        // vn>0 = bottom of sprite = bottom of face (vertex0/vertex3 side). V is inverted.
        int quadrant;
        if ((uSignum0 + uSignum1 + uSignum2 + uSignum3) <= 0) {
          // Left half of sprite (u < center)
          if ((vSignum0 + vSignum1 + vSignum2 + vSignum3) <= 0) {
            quadrant = 1; // top-left of sprite = vertex1's spatial position
          } else {
            quadrant = 0; // bottom-left of sprite = vertex0's spatial position
          }
        } else {
          // Right half of sprite (u > center)
          if ((vSignum0 + vSignum1 + vSignum2 + vSignum3) <= 0) {
            quadrant = 2; // top-right of sprite = vertex2's spatial position
          } else {
            quadrant = 3; // bottom-right of sprite = vertex3's spatial position
          }
        }
  ...
  protected void tryInterpolate(
      MutableQuadView quad, TextureAtlasSprite oldSprite, int spriteIndex) {
  ...
  public static class VertexContainer {
    ...
    public final Vertex[] lerpedVertices = new Vertex[] {vertex01, vertex12, vertex23, vertex30};
  ...
  public static class Factory implements QuadProcessor.Factory<CompactConnectingCtmProperties> {
    @Override
    public QuadProcessor createProcessor(
        CompactConnectingCtmProperties properties,
          ...
          if (key < replacementTextureAmount) {
            ...
            ContinuityClient.LOGGER.warn(
                "Cannot replace tile "
                    ...
                    + "' only supports "
                    + replacementTextureAmount
                    + " replacement tiles in file '"
                    + properties.getResourceId()
                    + "' in pack '"
                    + properties.getPackId()
                    + "'");
    ...
    public QuadProcessor createProcessor(
      ...
      return new CompactCtmQuadProcessor(
          ...
          properties.getConnectionPredicate(),
          properties.getInnerSeams(),
