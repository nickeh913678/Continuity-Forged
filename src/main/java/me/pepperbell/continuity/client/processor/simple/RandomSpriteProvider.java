...
public class RandomSpriteProvider implements SpriteProvider {
  ...
  public RandomSpriteProvider(
      TextureAtlasSprite[] sprites,
      RandomIndexProvider indexProvider,
      int randomLoops,
      Symmetry symmetry,
      boolean linked) {
  ...
  public TextureAtlasSprite getSprite(
    ...
    if (linked) {
      ...
      BlockPos.MutableBlockPos mutablePos =
          dataProvider.getData(ProcessingDataKeys.MUTABLE_POS).set(pos);
        ...
        i++;
          ...
          && block
              == blockView
                  ...
                  .getAppearance(blockView, mutablePos, face, state, pos)
                  .getBlock());
      y = mutablePos.getY() + 1;
    }
  ...
  public static class Factory implements SpriteProvider.Factory<RandomCtmProperties> {
    ...
    public SpriteProvider createSpriteProvider(
        TextureAtlasSprite[] sprites, RandomCtmProperties properties) {
      if (sprites.length == 1) {
        return new FixedSpriteProvider(sprites[0]);
      }
      return new RandomSpriteProvider(
          sprites,
          properties.getIndexProviderFactory().createIndexProvider(sprites.length),
          properties.getRandomLoops(),
          properties.getSymmetry(),
          properties.getLinked());
    }
