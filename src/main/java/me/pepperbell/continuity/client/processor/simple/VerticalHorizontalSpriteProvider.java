...
public class VerticalHorizontalSpriteProvider extends VerticalSpriteProvider {
  ...
  // 1   *   8
  // 2       4
  protected static final int[] SECONDARY_SPRITE_INDEX_MAP =
      new int[] {
  ...
  public VerticalHorizontalSpriteProvider(
  ...
  protected int getSecondaryConnections(
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (connectionPredicate.shouldConnect(
          blockView, state, appearanceState, pos, mutablePos, face, quadSprite, innerSeams)) {
        connections |= 1 << (i * 3);
        for (int j = 0; j < 2; j++) {
          mutablePos.setWithOffset(pos, direction).move(directions[((i + j) % 2) * 2 + 1]);
          if (connectionPredicate.shouldConnect(
              blockView, appearanceState, state, pos, mutablePos, face, quadSprite, innerSeams)) {
  ...
  public static class Factory implements SpriteProvider.Factory<OrientedConnectingCtmProperties> {
    ...
    public SpriteProvider createSpriteProvider(
        TextureAtlasSprite[] sprites, OrientedConnectingCtmProperties properties) {
      return new VerticalHorizontalSpriteProvider(
          sprites,
