...
public class BaseCachingPredicates implements CachingPredicates {
  @Nullable protected Set<ResourceLocation> spriteIdSet;
  @Nullable protected Predicate<BlockState> blockStatePredicate;
  protected boolean isValidForMultipass;

  public BaseCachingPredicates(
      @Nullable Set<ResourceLocation> spriteIdSet,
      @Nullable Predicate<BlockState> blockStatePredicate,
  ...
  public static class Factory<T extends BaseCtmProperties> implements CachingPredicates.Factory<T> {
    ...
    @Override
    public CachingPredicates createPredicates(
        T properties, Function<Material, TextureAtlasSprite> textureGetter) {
      return new BaseCachingPredicates(
          properties.getMatchTilesSet(), properties.getMatchBlocksPredicate(), isValidForMultipass);
    }
  }
}
